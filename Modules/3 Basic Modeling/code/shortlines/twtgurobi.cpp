    ...
    try
    {
        ...
        for (int i = 0; i < nbJobs; ++i)
        {
            for (int j = i+1; j < nbJobs; ++j)
            {
                ...
                varname << "x(" << i << ")(" << j << ")";
                x.emplace(std::make_tuple(i, j),
                    model.addVar(0.0, 1.0, 0.0, GRB_BINARY, varname.str()));
            }
        }
        ...
        for (int j = 0; j < nbJobs; ++j)
        {
            std::ostringstream varname;
            varname << "startTime(" << j << ")";
            startTime.emplace_back(
                model.addVar(0.0, GRB_INFINITY, 0.0, GRB_CONTINUOUS, varname.str()));
        }
        ...
        for (auto& kv : x)
        {
            ...
            model.addConstr(startTime[j] >= startTime[i] + duration[i] - M*(1-x_ij),
                consname.str());
            consname.str("");
            consname << "NoOverlap2_" << i << "_" << j;
            model.addConstr(startTime[i] >= startTime[j] + duration[j] - M*x_ij,
                consname.str());
        }
        for (int i = 0; i < nbJobs; ++i)
        {
            ...
            model.addConstr(tardiness[i] >= startTime[i] + duration[i] - deadline[i],
                consname.str());
        }
